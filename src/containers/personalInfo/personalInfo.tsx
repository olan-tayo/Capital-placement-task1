...
export const PersonalInfomation = ({ request }: any) => {
  const time = [
    {
      title: "Minute",
    },
  ...
  const [data, setData] = useState<Data[]>(
    request?.personalInformation?.personalQuestions
  );
  ...
  const handleToggleSwitch = (index: number) => {
    ...
    updatedPersonalInfo[index].show = !updatedPersonalInfo[index].show;
  ...
  const handleInternalCheckbox = async (index: number, id: string) => {
    const updatedPersonalInfo = [...personalInfo];
    updatedPersonalInfo[index].internalUse =
      !updatedPersonalInfo[index].internalUse;
    setPersonalInfo(updatedPersonalInfo);

    const updatedData = personalInfo?.map((data) => {
      if (data?.id === id) {
        return {
          ...data,
    ...
    try {
      await updateData(updatedRequest);
    } catch (error) {
      // console.log(error);
    }
  };
  ...
  const handleUpdateQuestions = async (updatedData: Data[]) => {
    ...
    const updatedRequest = {
      ...
      personalInformation: {
        ...request.personalInformation,
        personalQuestions: newUpdatedData,
      },
    };
  ...
  useEffect(() => {
    setData(request?.personalInformation?.personalQuestions);
  }, [request]);
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    <div className="flex gap-6 items-center">
                      <div className="flex items-center gap-4">
                        <input
                          type="checkbox"
                          checked={info?.internalUse}
                          onChange={() =>
                            handleInternalCheckbox(index, info?.id)
                          }
